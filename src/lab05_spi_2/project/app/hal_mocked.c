...
#define CHAR_DC1       0x11u
#define CHAR_DC2       0x12u
#define CHAR_ESC       0x1Bu
#define CHAR_ACK       0x06u
#define CHAR_NAK       0x15u
#define ABORT          0x04u
#define BUTTON_MASK_T0 0x01u
...
static void print_ct_lcd(char *buf, uint8_t offset);
...
{
    uint8_t result = out;
    assert(state == Idle || state == Cmd || state == Data || 
            state == Resp || state == Bcc);
    switch(state) {
        case Idle:
...
static void error(void)
{
    print_ct_lcd("Protocol error.", 20);
    out = CHAR_NAK;
    state = Idle;
}
    ...
    if (cmd == CHAR_DC1 || cmd == CHAR_DC2) {
        if (cmd == CHAR_DC1) {
            ...
            if (cmd_buf.len > 7 && 0 == strncmp((char *)cmd_buf.bytes, "\x1bZL", 3)) {
                print_ct_lcd(((char *)cmd_buf.bytes+7), 0);
                print_ct_lcd("print_text", 20);
            }
            ok_status = 1;
...
static uint8_t data_at(data_t *data, uint8_t position)
{
